...
const TenantsPage = () => {
    ...
    const fetchRooms = async () => {
        try {
            ...
            alert(`Error fetching rooms: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    };
    ...
    const fetchUserById = async (userId: string) => {
        try {
            ...
            alert(`Error fetching user: ${error instanceof Error ? error.message : "Unknown error"}`);
    ...
    const filteredRooms = rooms.filter(room =>
        ...
        (room.renterID && (currentTenants[room.id]?.firstname.toLowerCase().includes(searchTerm.toLowerCase()) || currentTenants[room.id]?.lastname.toLowerCase().includes(searchTerm.toLowerCase())))
    ...
    const handleSaveTenantChanges = async () => {
        ...
        try {
           ...
           setRooms(prevRooms => prevRooms.map(r =>
                r.id === room.id ? { ...r, renterID: inputTenantId === "unassigned" ? null : inputTenantId } : r
            ));
            if (!inputTenantId || inputTenantId === "unassigned") {
                setTenantDetails(null);
    ...
    const confirmDeleteTenant = async () => {
        ...
        try {
            ...
            setRooms(prevRooms => prevRooms.map(r =>
                r.id === room.id ? { ...r, renterID: null } : r
            ));
            setTenantDetails(null);
            setCurrentTenants(prev => {
                const updated = { ...prev };
        ...
        <div className="min-h-screen bg-gray-50 flex flex-col overflow-auto">
                                    ...
                                    <div key={floor} className="mb-12">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                            {roomsByFloor[floor].map((room) => (
                                                <Card
                                                    key={room.id}
                                                    className={`hover:shadow-md transition-all duration-200 cursor-pointer ${room.renterID ? "" : "bg-gray-50"}`}
                                                ...
                                                >
                                                    <CardHeader className="pb-2">
                                                        <div className="flex justify-between items-start">
                                                            ...
                                                            <Badge
                                                                variant="outline"
                                                                className={`${room.renterID
                                                                    ? "bg-green-50 text-green-700 border-green-200"
                                                                    : "bg-gray-50 text-gray-700 border-gray-200"
                                                                    }`}
                                                            >
                                                                {room.renterID ? "Occupied" : "Vacant"}
                                                            </Badge>
                                                        </div>
                                                    </CardHeader>
                                                    <CardContent>
                                                            ...
                                                            {room.renterID ? (
                                                               <div className="flex items-center gap-2 mt-2">
                                                                     <User className="h-4 w-4 text-gray-500" />
                                                                        ...
                                                                        {currentTenants[room.id]?.firstname} {currentTenants[room.id]?.lastname}
                                                                        </span>
                                                                  </div>
                                                            ) : (
            ...
            <Dialog open={isUserDialogOpen} onOpenChange={setIsUserDialogOpen}>
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        <DialogTitle>
                            {selectedRoomId && `Room ${rooms.find(r => r.id === selectedRoomId)?.roomName} - ${rooms.find(r => r.id === selectedRoomId)?.description}`}
                        </DialogTitle>
                        <DialogDescription>
                    ...
                    <div className="space-y-4 py-2">
                        {selectedRoomId && !rooms.find(r => r.id === selectedRoomId)?.renterID && !editingTenant && (
                            <div className="rounded-md bg-gray-50 p-4 flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-gray-600">You can assign a tenant by clicking "Edit Tenant" below</p>
                                </div>
                            </div>
                        )}
                        ...
                        {!editingTenant && tenantDetails && (
                            <div className="space-y-4">
                                <div className="rounded-md bg-blue-50 p-4 flex items-center gap-3">
                                    ...
                                    <div>
                                        <p className="font-medium text-blue-800">{tenantDetails.firstname} {tenantDetails.lastname}</p>
                                ...
                                {tenantDetails.address && (
                                    <div className="flex items-center gap-2">
                                        ...
                                        <p className="text-sm text-gray-600">{tenantDetails.address}</p>
                                    </div>
                                )}
                                {tenantDetails.telephone && (
                                    <div className="flex items-center gap-2">
                                        <Phone className="h-4 w-4 text-gray-600" />
                                        <p className="text-sm text-gray-600">{tenantDetails.telephone}</p>
                                    </div>
                                )}
                                {tenantDetails.email && (
                                    <div className="flex items-center gap-2">
                                        <Mail className="h-4 w-4 text-gray-600" />
                                        <p className="text-sm text-gray-600">{tenantDetails.email}</p>
                                    </div>
                                )}
                                {tenantDetails.userimg && (
                                    <div className="flex items-center gap-2">
                                        <User className="h-4 w-4 text-gray-600" />
                                        <img src={tenantDetails.userimg} alt="User" className="h-10 w-10 rounded-full" />
                                    </div>
                                )}
                            </div>
                        )}
                    ...
                    <DialogFooter>
                        {editingTenant ? (
                            <>
                                <Button variant="outline" onClick={() => setEditingTenant(false)}>Cancel</Button>
                            ...
                            <>
                                ...
                                <Button onClick={handleEditTenant}>Edit Tenant</Button>
                            </>
                        )}
                    </DialogFooter>
                </DialogContent>

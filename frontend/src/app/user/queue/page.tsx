...
const QueueAppointment = () => {
  ...
  const filteredRoomTypes = roomTypes.filter(room =>
    ...
    const checkQueueStatus = async (roomType: string, id: number) => {
        try {
            const response = await fetch(`http://localhost:3000/queue/check/${roomType}`, {
                method: 'GET',
                headers: {
  ...
  const handleConfirmAppointment = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/queue/${selectedRoomType.roomType}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        ...
        if (response.status === 409) {
           ...
           toast.error("คิวเต็ม", {
             description: "คิวสำหรับห้องประเภทนี้เต็มแล้ว กรุณาเลือกห้องประเภทอื่นหรือลองใหม่ภายหลัง",
             duration: 5000,
           });
          ...
          toast.error("กรุณา login", {
           ...
           toast.error("เกิดข้อผิดพลาด", {
             description:  errorData.message || "มีข้อผิดพลาดในการจองคิว กรุณาลองอีกครั้ง",
             duration: 5000
          });
        }
                  ...
                  <Card key={roomType.id} className="overflow-hidden hover:shadow-md transition-shadow duration-300">
                    ...
                    <CardHeader className="pb-2">
                      <div className="flex justify-between items-center">
                        <CardTitle className="text-xl">Type {roomType.roomType}</CardTitle>
                      </div>
